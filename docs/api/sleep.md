...
오늘 밤 추천 수면 계획(`SleepPage`의 "오늘 밤 추천 수면 시간" 카드). 최근 7일 평균 수면 시간(3일 이상 기록이 있을 때) 또는 `goalHours`(부족할 때)를 목표 수면 시간으로 삼고, 내일 가장 이른 `schedule_task` 시작 시각을 기상 시각의 제약으로 반영해 계산한다. 결과는 하루 1회 `sleep_plan` 테이블에 캐시되어 같은 날 재요청 시 그대로 재사용된다.
