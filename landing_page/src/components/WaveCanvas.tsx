...
export default function WaveCanvas() {
  ...
  useEffect(() => {
    ...
    const glContext = canvas.getContext("webgl2", { antialias: false, alpha: false });
    ...
    const uniformNames = ["uVideo", "uHeight", "uSim", "uVidScale", "uVidOffset", "uRefr", "uSpec"] as const;
    ...
    const sinLut = (t: number) => SIN[(t | 0) & 1023];
    ...
    function updateCover() {
      ...
      gl.uniform2f(U.uVidScale, sx, sy);
      gl.uniform2f(U.uVidOffset, (1 - sx) / 2, (1 - sy) / 2);
    }
    ...
    function disturb(cx: number, cy: number, r: number, force: number) {
      const r2 = r * r;
      for (let y = -r; y <= r; y++) {
        for (let x = -r; x <= r; x++) {
          const d2 = x * x + y * y;
    ...
    function stepWater(time: number) {
      frameNo++;
