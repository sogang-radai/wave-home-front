...
function App() {
  ...
  const [homeTab, setHomeTab] = useState('trigger');
  const [sleepTab, setSleepTab] = useState('analysis');
  const [powerTab, setPowerTab] = useState('analysis');

  // Lets the sidebar show/select the current page's sub-tabs without knowing
  // each page's tab state variable by name.
  const sidebarTabState = {
    sleep: [sleepTab, setSleepTab],
    power: [powerTab, setPowerTab],
    home: [homeTab, setHomeTab],
  };
  ...
  return (
    <div className="app-shell">
      ...
      <Sidebar
        page={page}
        onSelect={setPage}
        activeTab={sidebarTabState[page]?.[0]}
        onSelectTab={(tabId) => sidebarTabState[page]?.[1]?.(tabId)}
      ...
      <section className="workspace">
        ...
        <main className={`content${page === 'chat' && chatMode === 'page' ? ' chat-active' : ''}${page === 'main' ? ' dashboard-active' : ''}`}>
          ...
          {page === 'power' && <PowerPage tab={powerTab} setTab={setPowerTab} />}
          {page === 'home' && (
            <HomeControlPage
              tab={homeTab}
              setTab={setHomeTab}
