...
export default function ValuePropSection() {
  ...
  useGSAP(
    () => {
      ...
      // Mobile: one-shot text entrance, no pin / Pixi shatter. Those fight
      // native scrolling and keep a particle ticker alive down the page.
      mm.add("(max-width: 1023px)", () => {
        gsap.set(line1, { opacity: 0, y: 12 });
        gsap.set(line2, { opacity: 0, xPercent: -6 });
        gsap.set(line3, { opacity: 0, y: 12 });
        gsap.set([shatterMount, overlay], { opacity: 0 });
        ...
        tl.to(line1, { opacity: 1, y: 0, duration: 0.45, ease: "power2.out" }, 0)
          .to(line2, { opacity: 1, xPercent: 0, duration: 0.55, ease: "power2.out" }, 0.08)
          .to(line3, { opacity: 1, y: 0, duration: 0.45, ease: "power2.out" }, 0.2);
