          ...
          <Card title="WaveAI 추천 일간 권장 액션">
            <div className="posture-action-list">
              {dailyInsights.map((item) => (
                <InsightCard key={item.id} id={item.id} approved={item.approved} actionable={item.actionable} label={item.label} kind={item.kind} title={item.title} text={item.text} actionType={item.actionType} scheduleTaskJson={item.scheduleTaskJson} ruleJson={item.ruleJson} onToggle={toggleInsight} />
              ))}
            </div>
          </Card>
